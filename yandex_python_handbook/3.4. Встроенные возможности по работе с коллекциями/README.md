...
for n in numbers:
    print(*[a * b for a, b in product(numbers, [n])])
```
...
goods = list(chain.from_iterable([input().split(", ") for _ in range(int(input()))]))
for index, value in enumerate(sorted(goods), 1):
    print(f"{index}. {value}")
```
...
for values in permutations(sorted(input() for _ in range(int(input())))):
    print(*values, sep=', ')  # print(", ".join(values))
```
...
for values in product((False, True), repeat=len(variables)):
    ...
    print(" ".join(map(lambda x: str(int(x)), (*values, f))))
```
